#!/usr/bin/env bash
# PortGremlin setup — interactive install for Virtual Lab or hardware flash.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VENV="${ROOT}/.venv"
TIVAWARE_PATH="${TIVAWARE_PATH:-/opt/ti/TivaWare_C_Series-2.2.0.295}"
MODE="${PORTGREMLIN_MODE:-}"

RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
BOLD='\033[1m'
NC='\033[0m'

info()  { echo -e "${CYAN}[setup]${NC} $*"; }
ok()    { echo -e "${GREEN}[ok]${NC} $*"; }
warn()  { echo -e "${YELLOW}[warn]${NC} $*"; }
fail()  { echo -e "${RED}[error]${NC} $*" >&2; exit 1; }

have() { command -v "$1" >/dev/null 2>&1; }

print_banner() {
    echo ""
    echo -e "${RED}  ██████╗  ██████╗ ██████╗ ████████╗ ██████╗ ██████╗ ███████╗███╗   ███╗██╗███╗   ██╗${NC}"
    echo -e "${RED}  ██╔══██╗██╔═══██╗██╔══██╗╚══██╔══╝██╔════╝ ██╔══██╗██╔════╝████╗ ████║██║████╗  ██║${NC}"
    echo -e "${RED}  ██████╔╝██║   ██║██████╔╝   ██║   ██║  ███╗██████╔╝█████╗  ██╔████╔██║██║██╔██╗ ██║${NC}"
    echo -e "${RED}  ██╔═══╝ ██║   ██║██╔══██╗   ██║   ██║   ██║██╔══██╗██╔══╝  ██║╚██╔╝██║██║██║╚██╗██║${NC}"
    echo -e "${RED}  ██║     ╚██████╔╝██║  ██║   ██║   ╚██████╔╝██║  ██║███████╗██║ ╚═╝ ██║██║██║ ╚████║${NC}"
    echo -e "${RED}  ╚═╝      ╚═════╝ ╚═╝  ╚═╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝${NC}"
    echo -e "${CYAN}  USB Enumeration Security Research Platform${NC}"
    echo ""
}

prompt_mode() {
    if [[ -n "${MODE}" ]]; then
        return 0
    fi
    echo -e "${BOLD}How do you want to run PortGremlin?${NC}"
    echo ""
    echo -e "  ${GREEN}1)${NC} Virtual Lab     GUI simulation — no hardware needed"
    echo -e "  ${GREEN}2)${NC} Hardware Flash  Build & flash TM4C123 LaunchPad"
    echo ""
    local choice
    while true; do
        read -r -p "Enter choice [1/2]: " choice
        case "${choice}" in
            1|virtual|Virtual|VIRTUAL)
                MODE="virtual"
                break
                ;;
            2|flash|Flash|FLASH|hardware)
                MODE="flash"
                break
                ;;
            *)
                echo "Please enter 1 or 2."
                ;;
        esac
    done
    echo ""
}

install_apt_deps() {
    local need_tk="${1:-0}"
    if ! have apt-get; then
        return 0
    fi
    local pkgs=()
    have python3    || pkgs+=(python3)
    have pip3       || pkgs+=(python3-pip)
    python3 -c "import venv" 2>/dev/null || pkgs+=(python3-venv)
    if [[ "${need_tk}" == "1" ]]; then
        python3 -c "import tkinter" 2>/dev/null || pkgs+=(python3-tk)
    fi
    if [[ "${MODE}" == "flash" ]]; then
        have lsusb || pkgs+=(usbutils)
    fi
    if ((${#pkgs[@]})); then
        info "Installing system packages: ${pkgs[*]}"
        sudo apt-get update -qq
        sudo apt-get install -y "${pkgs[@]}"
    fi
}

install_toolchain() {
    if have arm-none-eabi-gcc; then
        ok "arm-none-eabi-gcc: $(arm-none-eabi-gcc --version | head -1)"
        return 0
    fi
    if have apt-get; then
        info "Installing ARM GCC toolchain..."
        sudo apt-get install -y gcc-arm-none-eabi binutils-arm-none-eabi 2>/dev/null \
            && ok "ARM toolchain installed" \
            && return 0
    fi
    warn "arm-none-eabi-gcc not found — firmware build will be skipped"
}

install_flash_tools() {
    if have lm4flash; then
        ok "lm4flash found"
    elif have openocd; then
        ok "openocd found"
    else
        warn "No flash tool (lm4flash/openocd). Build only — flash manually."
    fi
}

setup_python() {
    if [[ ! -d "${VENV}" ]]; then
        info "Creating Python virtualenv at .venv"
        python3 -m venv "${VENV}"
    fi
    # shellcheck disable=SC1091
    source "${VENV}/bin/activate"
    pip install --upgrade pip -q
    pip install -r "${ROOT}/tools/requirements.txt" -q
    ok "Python deps installed"
}

build_firmware() {
    if ! have arm-none-eabi-gcc; then
        warn "Skipping firmware build (no toolchain)"
        return 0
    fi
    if [[ ! -d "${TIVAWARE_PATH}" ]]; then
        warn "Skipping firmware build — TivaWare not at ${TIVAWARE_PATH}"
        warn "  export TIVAWARE_PATH=/opt/ti/TivaWare_C_Series-2.2.0.295"
        return 0
    fi
    info "Building firmware..."
    make -C "${ROOT}/usb_dev_keyboard" TIVAWARE_PATH="${TIVAWARE_PATH}"
    ok "Firmware built: usb_dev_keyboard/portgremlin.bin"
}

flash_firmware() {
    if [[ ! -f "${ROOT}/usb_dev_keyboard/portgremlin.bin" ]]; then
        build_firmware
    fi
    if [[ ! -f "${ROOT}/usb_dev_keyboard/portgremlin.bin" ]]; then
        warn "No firmware binary to flash."
        return 0
    fi
    if have lm4flash; then
        read -r -p "Flash LaunchPad now? [y/N]: " ans
        if [[ "${ans}" =~ ^[Yy] ]]; then
            info "Flashing..."
            lm4flash "${ROOT}/usb_dev_keyboard/portgremlin.bin"
            ok "Flash complete"
        fi
    else
        warn "Flash manually: make -C usb_dev_keyboard flash"
    fi
}

setup_groups() {
    if have lsusb && ! groups | grep -q '\bplugdev\b'; then
        warn "Add yourself to plugdev: sudo usermod -aG plugdev \$USER"
    fi
    if [[ -e /dev/ttyACM0 ]] 2>/dev/null && ! groups | grep -q '\bdialout\b'; then
        warn "Add yourself to dialout: sudo usermod -aG dialout \$USER"
    fi
}

activate_venv() {
    if [[ ! -d "${VENV}" ]]; then
        fail "Virtualenv not found. Run ./setup.sh first."
    fi
    # shellcheck disable=SC1091
    source "${VENV}/bin/activate"
}

run_virtual() {
    activate_venv
    print_banner
    info "Launching PortGremlin Virtual Lab (GUI)..."
    echo ""
    exec python3 "${ROOT}/tools/portgremlin-simulator.py" "$@"
}

run_overwatch() {
    activate_venv
    print_banner
    info "Launching PortGremlin Overwatch..."
    info "Dashboard: http://127.0.0.1:8765"
    info "Connect LaunchPad: Device USB -> target, ICDI serial -> this machine"
    echo ""
    exec python3 "${ROOT}/tools/portgremlin-overwatch.py" "$@"
}

do_setup_virtual() {
    print_banner
    info "Mode: ${BOLD}Virtual Lab${NC} (GUI simulation)"
    install_apt_deps 1
    setup_python
    ok "Virtual Lab ready!"
    echo ""
    read -r -p "Launch Virtual Lab now? [Y/n]: " launch
    if [[ ! "${launch}" =~ ^[Nn] ]]; then
        run_virtual
    else
        echo "  Run later: ./setup.sh --run --virtual"
    fi
}

do_setup_flash() {
    print_banner
    info "Mode: ${BOLD}Hardware Flash${NC} (TM4C123 LaunchPad)"
    install_apt_deps 0
    install_toolchain
    install_flash_tools
    setup_python
    build_firmware
    setup_groups
    flash_firmware
    ok "Hardware setup complete!"
    echo ""
    read -r -p "Launch Overwatch monitor now? [Y/n]: " launch
    if [[ ! "${launch}" =~ ^[Nn] ]]; then
        run_overwatch
    else
        echo "  Run later: ./setup.sh --run --flash"
    fi
}

do_setup() {
    prompt_mode
    if [[ "${MODE}" == "virtual" ]]; then
        do_setup_virtual
    else
        do_setup_flash
    fi
}

do_run() {
    prompt_mode
    if [[ "${MODE}" == "virtual" ]]; then
        run_virtual "$@"
    else
        run_overwatch "$@"
    fi
}

main() {
    cd "${ROOT}"

    case "${1:-}" in
        --run)
            shift
            if [[ "${1:-}" == "--virtual" ]]; then
                MODE="virtual"
                shift
            elif [[ "${1:-}" == "--flash" ]]; then
                MODE="flash"
                shift
            fi
            do_run "$@"
            ;;
        --virtual)
            MODE="virtual"
            do_setup_virtual
            ;;
        --flash)
            MODE="flash"
            do_setup_flash
            ;;
        --help|-h)
            echo "Usage:"
            echo "  ./setup.sh                 Interactive setup (choose Virtual or Flash)"
            echo "  ./setup.sh --virtual       Setup Virtual Lab only"
            echo "  ./setup.sh --flash         Setup hardware toolchain + firmware"
            echo "  ./setup.sh --run           Interactive run"
            echo "  ./setup.sh --run --virtual Launch GUI simulator"
            echo "  ./setup.sh --run --flash   Launch Overwatch monitor"
            echo ""
            echo "Environment:"
            echo "  PORTGREMLIN_MODE=virtual|flash  Skip interactive prompt"
            echo "  TIVAWARE_PATH                   Path to TivaWare SDK"
            ;;
        *)
            do_setup
            ;;
    esac
}

main "$@"
